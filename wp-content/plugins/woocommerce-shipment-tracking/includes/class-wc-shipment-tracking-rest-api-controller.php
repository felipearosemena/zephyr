...
class WC_Shipment_Tracking_REST_API_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$tracking_items = $st->get_tracking_items( $order->ID, true );

		$data = array();
		foreach ( $tracking_items as $tracking_item ) {
			$tracking_item['order_id'] = $order->ID;
	...
	public function create_item( $request ) {
		...
		$st            = WC_Shipment_Tracking_Actions::get_instance();
		$tracking_item = $st->add_tracking_item( $order->ID, $args );

		$tracking_item['order_id'] = $order->ID;

		$formatted     = $st->get_formatted_tracking_item( $order->ID, $tracking_item );
		...
		$response->header( 'Location', rest_url( sprintf( '/%s/%s/%d', $this->namespace, str_replace( '(?P<order_id>[\d]+)', $order->ID, $this->rest_base ), $tracking_item['tracking_id'] ) ) );
	...
	public function get_item( $request ) {
		...
		$st            = WC_Shipment_Tracking_Actions::get_instance();
		$tracking_item = $st->get_tracking_item( $order->ID, $tracking_id, true );
	...
	public function delete_item( $request ) {
		...
		$result = $st->delete_tracking_item( $order->ID, $tracking_id );
		if ( ! $result ) {
