...
class WC_Shipping_Canada_Post extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->method_description = __( 'The <strong>Canada Post</strong> extension obtains rates dynamically from the Canada Post API during cart/checkout.', 'woocommerce-shipping-canada-post' );
	...
	private function environment_check() {
		...
		if ( get_woocommerce_currency() != "CAD" ) {
			echo '<div class="error">
				<p>' . __( 'Canada Post requires that the currency is set to Canadian Dollars.', 'woocommerce-shipping-canada-post' ) . '</p>
			</div>';
		}

		elseif ( $woocommerce->countries->get_base_country() != "CA" ) {
			echo '<div class="error">
				<p>' . __( 'Canada Post requires that the base country/region is set to Canada.', 'woocommerce-shipping-canada-post' ) . '</p>
			</div>';
		}

		elseif ( ! $this->origin && $this->enabled == 'yes' ) {
			echo '<div class="error">
				<p>' . __( 'Canada Post is enabled, but the origin postcode has not been set.', 'woocommerce-shipping-canada-post' ) . '</p>
			</div>';
		}
	}
    ...
    public function init_form_fields() {
    	...
    	$this->form_fields  = array(
			'enabled'          => array(
				'title'           => __( 'Enable/Disable', 'woocommerce-shipping-canada-post' ),
				'type'            => 'checkbox',
				'label'           => __( 'Enable this shipping method', 'woocommerce-shipping-canada-post' ),
				'default'         => 'no'
			),
			'title'            => array(
		    ...
		    'availability'  => array(
				'title'           => __( 'Method Availability', 'woocommerce-shipping-canada-post' ),
				'type'            => 'select',
				'default'         => 'all',
				'class'           => 'availability',
				'options'         => array(
					'all'            => __( 'All Countries', 'woocommerce-shipping-canada-post' ),
					'specific'       => __( 'Specific Countries', 'woocommerce-shipping-canada-post' ),
				),
			),
			'countries'        => array(
				'title'           => __( 'Specific Countries', 'woocommerce-shipping-canada-post' ),
				'type'            => 'multiselect',
				'class'           => 'chosen_select',
				'css'             => 'width: 450px;',
				'default'         => '',
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->length && $values['data']->height && $values['data']->width && $values['data']->weight ) {
				...
				if ( function_exists( 'wc_add_notice' ) ) {
					...
					$woocommerce->add_error( sprintf( __( 'Product # is missing dimensions! Using 1x1x1.', 'woocommerce-shipping-canada-post' ), $item_id ) );
				}
    ...
    private function per_item_shipping( $package ) {
    	...
    	foreach ( $package['contents'] as $item_id => $values ) {
			...
			$weight = round( wc_get_weight( $values['data']->get_weight(), 'kg' ), 2 );
			// See issue #21.
			if ( $weight < 0.01 ) {
				$weight = 0.01;
			}
			...
			if ( ! empty( $this->options ) ) {
				...
				foreach ( $this->options as $option ) {
					...
					if ( $option == 'COV' ) {
						$option_request .= '			<option-amount>' . $values['data']->get_price() . '</option-amount>' . "\n";
					}
					$option_request .= '		</option>' . "\n";
				}
				if ( $option_request ) {
    ...
    private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->length && $values['data']->height && $values['data']->width && $values['data']->weight ) {
				...
				if ( function_exists( 'wc_add_notice' ) ) {
					...
					$woocommerce->add_error( sprintf( __( 'Product # is missing dimensions. Aborting.', 'woocommerce-shipping-canada-post' ), $item_id ) );
				}
				return;
			}
		}
		...
		foreach ( $flat_packages as $flat_package ) {
			...
			$weight = round( $flat_package->weight, 2 );
			// See issue #21.
			if ( $weight < 0.01 ) {
				$weight = 0.01;
			}
